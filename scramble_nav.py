...
JUMP_TEMPLATES: List[JumpTemplate] = [
    ...
    # 전략: 플랫폼 밑으로 들어가지 말고, 플랫폼 '옆'에서 안쪽으로 뛴다.
    # ==========================================================

    # [Left Corner] r3_L1으로 갈 때
    # 이전: 0.05~0.12 (완전 구석탱이, 플랫폼 밑)
    # 수정: 0.16~0.20 (플랫폼보다 살짝 오른쪽 바깥)에서 왼쪽(-1)으로 점프
    JumpTemplate("floor_to_r3_L1_side", "floor", "r3_L1",
                 0.16, 0.20,  # 여기가 "검은 선" 목표가 됨 (약 x=200~250 부근)
                 0.5, 0.9,  # 착지는 플랫폼의 오른쪽 끝 쯤
                 -1,  # 왼쪽으로 점프!
                 0.35),

    # [Right Corner] r3_R2로 갈 때
    # 수정: 0.80~0.84 (플랫폼보다 살짝 왼쪽 바깥)에서 오른쪽(+1)으로 점프
    ...
    # 전략: 맵의 정중앙(Center Gap) 쪽으로 걸어와서 바깥으로 뛴다.
    # ==========================================================

    # [Mid Left] r3_L2로 갈 때
    # 플랫폼 밑(0.2~0.3)이 아니라, 중앙 통로(0.40 부근)까지 걸어와서 뛴다.
    JumpTemplate("floor_to_r3_L2_center_entry", "floor", "r3_L2",
                 0.38, 0.43,  # 맵 중앙보다 살짝 왼쪽 빈 공간
                 0.6, 0.9,  # 플랫폼의 오른쪽 끝에 착지
                 -1,  # 왼쪽으로 점프
                 0.4),  # 거리가 좀 있으니 0.4초 꾹

    # [Mid Right] r3_R1로 갈 때
    JumpTemplate("floor_to_r3_R1_center_entry", "floor", "r3_R1",
                 0.57, 0.62,  # 맵 중앙보다 살짝 오른쪽 빈 공간
                 0.1, 0.4,  # 플랫폼 왼쪽 끝 착지
                 +1,  # 오른쪽으로 점프
                 0.4),
    ...
    # 3. 1층끼리 이동 (징검다리)
    # ==========================================================
    JumpTemplate("r3_L1_to_r3_L2", "r3_L1", "r3_L2", 0.7, 0.95, 0.1, 0.3, +1, 0.35),
    JumpTemplate("r3_L2_to_r3_L1", "r3_L2", "r3_L1", 0.05, 0.3, 0.7, 0.9, -1, 0.35),
    JumpTemplate("r3_R2_to_r3_R1", "r3_R2", "r3_R1", 0.05, 0.3, 0.7, 0.9, -1, 0.35),
    JumpTemplate("r3_R1_to_r3_R2", "r3_R1", "r3_R2", 0.7, 0.95, 0.1, 0.3, +1, 0.35),

    # ==========================================================
    # 4. 1층 -> 2층 (High Platforms)
    # ==========================================================
    # r3_L2 -> r2_L (위층으로)
    # 바로 위가 아니라 대각선으로
    JumpTemplate("r3_L2_to_r2_L", "r3_L2", "r2_L",
                 0.7, 0.9,  # r3_L2의 오른쪽 끝에서
                 0.7, 0.9,  # r2_L의 오른쪽 끝으로
                 0,  # 수직 점프 (여긴 겹쳐있어서 수직이 나을 수 있음, 안되면 -1)
                 0.45),  # 높으니까 0.45초

    # r3_R1 -> r2_R
    JumpTemplate("r3_R1_to_r2_R", "r3_R1", "r2_R",
                 0.1, 0.3,
                 0.1, 0.3,
                 0,
                 0.45),
    ...
    # 5. 최상층 및 하강
    # ==========================================================
    JumpTemplate("r2_L_to_mid_top", "r2_L", "mid_top", 0.6, 0.9, 0.1, 0.4, +1, 0.45),
    JumpTemplate("r2_R_to_mid_top", "r2_R", "mid_top", 0.1, 0.4, 0.6, 0.9, -1, 0.45),
