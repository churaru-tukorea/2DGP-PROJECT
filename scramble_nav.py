...
JUMP_TEMPLATES: List[JumpTemplate] = [
    # ==========================================
    # 1. 1층 왼쪽 중앙 (r3_L2) 접근 경로 (3가지)
    # ==========================================
    # [A] 수직 점프 (바로 아래에서)
    JumpTemplate("floor_to_r3_L2_vertical", "floor", "r3_L2",
                 0.28, 0.34, 0.2, 0.8, 0, 0.6),
    # [B] 좌측에서 우측으로 점프 (Left Approach)
    JumpTemplate("floor_to_r3_L2_from_left", "floor", "r3_L2",
                 0.15, 0.25, 0.1, 0.4, +1, 0.5),
    # [C] 우측에서 좌측으로 점프 (Right Approach) - 중앙 센터에서 진입
    JumpTemplate("floor_to_r3_L2_from_right", "floor", "r3_L2",
                 0.40, 0.50, 0.6, 0.9, -1, 0.5),

    # ==========================================
    # 2. 1층 오른쪽 중앙 (r3_R1) 접근 경로 (3가지)
    # ==========================================
    # [A] 수직 점프
    JumpTemplate("floor_to_r3_R1_vertical", "floor", "r3_R1",
                 0.66, 0.72, 0.2, 0.8, 0, 0.6),
    # [B] 좌측에서 우측으로 점프 (중앙 센터에서 진입)
    JumpTemplate("floor_to_r3_R1_from_left", "floor", "r3_R1",
                 0.50, 0.60, 0.1, 0.4, +1, 0.5),
    # [C] 우측에서 좌측으로 점프
    JumpTemplate("floor_to_r3_R1_from_right", "floor", "r3_R1",
                 0.75, 0.85, 0.6, 0.9, -1, 0.5),

    # ==========================================
    # 3. 구석 플랫폼들
    # ==========================================
    JumpTemplate("floor_to_r3_L1", "floor", "r3_L1", 0.01, 0.08, 0.2, 0.8, 0, 0.5),
    JumpTemplate("floor_to_r3_R2", "floor", "r3_R2", 0.92, 0.99, 0.2, 0.8, 0, 0.5),

    # ==========================================
    # 4. 층간 이동 (징검다리) - 상호 연결
    # ==========================================
    JumpTemplate("r3_L1_to_r3_L2", "r3_L1", "r3_L2", 0.6, 0.9, 0.1, 0.4, +1, 0.4),
    JumpTemplate("r3_L2_to_r3_L1", "r3_L2", "r3_L1", 0.1, 0.4, 0.6, 0.9, -1, 0.4),
    JumpTemplate("r3_R2_to_r3_R1", "r3_R2", "r3_R1", 0.1, 0.4, 0.6, 0.9, -1, 0.4),
    JumpTemplate("r3_R1_to_r3_R2", "r3_R1", "r3_R2", 0.6, 0.9, 0.1, 0.4, +1, 0.4),

    # 1층 중앙끼리 연결 (가운데 낭떠러지 넘기)
    #JumpTemplate("r3_L2_to_r3_R1", "r3_L2", "r3_R1", 0.8, 1.0, 0.0, 0.2, +1, 0.6),
    #JumpTemplate("r3_R1_to_r3_L2", "r3_R1", "r3_L2", 0.0, 0.2, 0.8, 1.0, -1, 0.6),

    # ==========================================
    # 5. 1층 -> 2층 (High Platforms)
    # ==========================================
    JumpTemplate("r3_L1_to_r2_L", "r3_L1", "r2_L", 0.2, 0.8, 0.1, 0.5, 0, 0.55),
    JumpTemplate("r3_L2_to_r2_L", "r3_L2", "r2_L", 0.1, 0.4, 0.5, 0.9, -1, 0.55),
    JumpTemplate("r3_R2_to_r2_R", "r3_R2", "r2_R", 0.2, 0.8, 0.5, 0.9, 0, 0.55),
    JumpTemplate("r3_R1_to_r2_R", "r3_R1", "r2_R", 0.6, 0.9, 0.1, 0.5, +1, 0.55),

    # ==========================================
    # 6. 2층 -> 3층 (Top)
    # ==========================================
    JumpTemplate("r2_L_to_mid_top", "r2_L", "mid_top", 0.7, 0.95, 0.1, 0.4, +1, 0.4),
    JumpTemplate("r2_R_to_mid_top", "r2_R", "mid_top", 0.05, 0.3, 0.6, 0.9, -1, 0.4),

    # ==========================================
    # 7. 하강 (Drop)
    # ==========================================
    JumpTemplate("drop_mid_to_r2_L", "mid_top", "r2_L", 0.0, 0.2, 0.6, 0.9, -1, 0.2),
    JumpTemplate("drop_mid_to_r2_R", "mid_top", "r2_R", 0.8, 1.0, 0.1, 0.4, +1, 0.2),
    JumpTemplate("drop_r2_L_to_r3_L1", "r2_L", "r3_L1", 0.0, 0.3, 0.2, 0.8, 0, 0.1),
    JumpTemplate("drop_r2_R_to_r3_R2", "r2_R", "r3_R2", 0.7, 1.0, 0.2, 0.8, 0, 0.1),
    JumpTemplate("drop_r3_L1_to_floor", "r3_L1", "floor", 0.0, 1.0, 0.08, 0.12, 0, 0.0),
    JumpTemplate("drop_r3_R2_to_floor", "r3_R2", "floor", 0.0, 1.0, 0.88, 0.92, 0, 0.0),
    JumpTemplate("drop_r3_L2_to_floor", "r3_L2", "floor", 0.0, 1.0, 0.3, 0.4, 0, 0.0),
    JumpTemplate("drop_r3_R1_to_floor", "r3_R1", "floor", 0.0, 1.0, 0.6, 0.7, 0, 0.0),
]
