...
JUMP_TEMPLATES: List[JumpTemplate] = [
    ...
    # [전략] 플랫폼 안쪽 빈 공간에서 바깥쪽 구석을 향해 뛴다.
    # ==========================================================

    # [Left] 바닥 -> r3_L1 (왼쪽 구석)
    # 위치: 0.18~0.24 (플랫폼 오른쪽 허공) -> 왼쪽(-1) 점프
    ...
    # 위치: 0.76~0.82 (플랫폼 왼쪽 허공) -> 오른쪽(+1) 점프
    # (좌측 0.18~0.24와 대칭되는 위치입니다)
    ...
    # [전략] 맵 정중앙(Center)에서 양옆으로 갈라져 뛴다.
    # ==========================================================

    # [Left] 바닥 -> r3_L2 (중앙 왼쪽)
    # 위치: 0.40~0.45 (중앙보다 살짝 왼쪽) -> 왼쪽(-1) 점프
    ...
    # [Right] 바닥 -> r3_R1 (중앙 오른쪽)
    # 위치: 0.55~0.60 (중앙보다 살짝 오른쪽) -> 오른쪽(+1) 점프
    # (기존 값보다 살짝 더 왼쪽으로 당겨서 확실히 허공에서 뛰게 함)
    ...
    # ==========================================================
    # 3. 1층 플랫폼 간 이동 (징검다리)
    # [전략] 플랫폼 끝(Edge)에서 반대편으로 대각선 점프
    # ==========================================================

    # L1 <-> L2 (좌측)
    JumpTemplate("r3_L1_to_r3_L2", "r3_L1", "r3_L2", 0.75, 0.95, 0.05, 0.25, +1, 0.4),
    JumpTemplate("r3_L2_to_r3_L1", "r3_L2", "r3_L1", 0.05, 0.25, 0.75, 0.95, -1, 0.4),

    # R1 <-> R2 (우측) - 좌측과 완전 대칭으로 수정
    JumpTemplate("r3_R2_to_r3_R1", "r3_R2", "r3_R1", 0.05, 0.25, 0.75, 0.95, -1, 0.4),
    JumpTemplate("r3_R1_to_r3_R2", "r3_R1", "r3_R2", 0.75, 0.95, 0.05, 0.25, +1, 0.4),

    # ==========================================================
    # 4. 1층 -> 2층 (High Platforms: r2_L, r2_R)
    # [전략] '바깥쪽 끝'에서 안쪽으로 찔러 들어가는 대각선 점프
    # ==========================================================

    # [Left] r3_L2 -> r2_L
    # r3_L2의 오른쪽 끝(0.75~0.95)에서 왼쪽(-1)으로 점프
    ...
    # [Right] r3_R1 -> r2_R
    # r3_R1의 왼쪽 끝(0.05~0.25)에서 오른쪽(+1)으로 점프
    # (좌측과 대칭: 0.75~0.95 <-> 0.05~0.25)
